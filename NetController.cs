...
namespace ERIUnitySimpleServer
{
    public class NetController
    {
        ...
        private static Dictionary<EndPoint, byte[]> clientPointDic = new Dictionary<EndPoint, byte[]>(NetConstant.MaxClientCount);
        ...
        private static void JoinMethod(Packet packet, EndPoint clientEndPoint)
        {
            ...
            if (joinDic.Count >= NetConstant.MaxClientCount && !joinDic.Any((v) => { return v.Value == 0; }))
            {
                battleStarted = true;
                lock (sendQueue)
                {
